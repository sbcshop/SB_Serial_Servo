...
############# Left Frame ######################################################
        ...
        tk.Label(self, text="SB Serial Servo", bg="gray85",
                 fg='SteelBlue',
                 font=("Helvetica", 20)).place(x=10, y=0)
...
########## Com Port Block setup ###############
        ...
        com_label = tk.Label(serial_box, fg="Black",
                             text="COM:",
        ...
        self.com_entry = ttk.Combobox(serial_box, values=serial_ports(),
                                     width=13, font=self.label_font)
        self.com_entry.grid(row=1, column=2)
        

        baud_label = tk.Label(serial_box, fg="Black",
                              text="Baudrate",
                              font=self.label_font)
        baud_label.grid(row=2, column=0)

        baud_entry = tk.Entry(serial_box, width=15, font=self.label_font)
        ...
        self.led = tk_tools.Led(serial_box, size=30)
        self.led.grid(row=3, column=0)        
        ...
        label = tk.Label(self.left_frame, image=logo, height=45, width=205,
                         bg='white')
        label.place(x=2, y=400)
##################################################
        
        self.refresh_button = tk.Button(serial_box, text="Refresh Port",
                                        bg="gray90",
                                        font=self.label_font,
                                        command=self.check_port)
        self.refresh_button.grid(row=4, column=0, columnspan=3, sticky='nesw')

    def check_port(self):
        self.com_entry.config(values=serial_ports())

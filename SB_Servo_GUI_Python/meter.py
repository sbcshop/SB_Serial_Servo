...
def draw_dial(canv, x0, y0, degree, t, r):
    ...
    canv.create_oval(xr - 5, yr - 5, xr + 5, yr + 5, fil="#00A2E8",
                     tags=('dial', 'three'))
...
class PositionMeter(Initialize):
    ...
    def draw_PosMeter(self, divisions=100):
        self.canvas.create_arc(self.size / 5, self.size / 6,
                               self.size - self.size / 6,
                               self.size - self.size / 6, style="arc",
                               outline="red",
                               start=-61, extent=61, width=self.size / 10)
        self.canvas.create_arc(self.size / 6, self.size / 6,
                               self.size - self.size / 6,
                               self.size - self.size / 6, outline="orange",
                               start=0,
                               width=self.size / 10, style="arc", extent=60)

        self.canvas.create_arc(self.size / 6, self.size / 6,
                               self.size - self.size / 6,
                               self.size - self.size / 6, width=self.size / 10,
                               style="arc",
        ...
        self.readout = self.canvas.create_text(self.size / 2,
                                               4 * self.size / 5,
                                               font=(
                                               "Arial", int(self.size / 18),
                                               'bold'),
                                               fill="black", text='')
    ...
    def draw_tick(self, divisions=100):
        ...
        label = 'Position'
        self.canvas.create_text(self.size / 2, 2 * self.size / 5,
        ...
        for tick in range(divisions + 1):
            ...
            if (tick % 10) == 0:
                ...
                label = str(int(self.min_value + tick * (
                            self.max_value - self.min_value) / 100))
                self.canvas.create_text(x, y,
                                        font=("Arial", int(self.size / 20)),
                                        fill="black",
                                        text=label)
    ...
    def set_value(self, number: (float, int)):
        ...
        degree = 30.0 + (number - self.min_value) / (
                    self.max_value - self.min_value) * 300.0
        draw_dial(self.canvas, self.size / 2, self.size / 2, -1 * degree,
                  self.size / 3, 8)
...
class VoltageMeter(Initialize):
    ...
    def draw_VolMeter(self, divisions=100):
        self.canvas.create_arc(self.size / 7, self.size / 7,
    ...
    def draw_tick(self, divisions=100):
        ...
        outer_tick_radius = int((self.size - self.size / 9) * 0.45)
        label = "Voltage"
        self.canvas.create_text(self.size / 2, 3 * self.size / 10, text=label,
                                angle=0,
                                font=("Arial", int(self.size / 20)),
                                fill="red")
        self.readout = self.canvas.create_text(self.size / 2,
    ...
    def set_value(self, number: (float, int)):
        number = number if number <= self.max_value else self.max_value
        number = number if number > self.min_value else self.min_value
        degree = 90.0 + (number - self.min_value) / (
                    self.max_value - self.min_value) * 180.0
        draw_dial(self.canvas, self.size / 2, self.size / 2, -1 * degree,
                  self.size / 3, 8)
        label = str(float(number))
        self.canvas.delete(self.readout)
        self.readout = self.canvas.create_text(self.size / 3 + 35,
                                               self.size / 2 + 15,
                                               font=(
                                               "Arial", int(self.size / 19),
                                               'bold'),
                                               fill="black", text=label,
                                               angle=0)
